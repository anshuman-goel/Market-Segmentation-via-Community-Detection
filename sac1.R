...
# for(i in 1:length(colnames(attrib)))
# {
#   graph<-set_vertex_attr(graph,name=colnames(attrib)[i],value=attrib[,colnames(attrib)[i]])
# }
...
# modularity_eval<-function(i,community,total_vertexes)
# {
#   #community[]
# }
...
phase2<-function()
{
  ...
  while(length(unique(initial))>16)
  {
